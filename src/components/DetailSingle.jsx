...
import { storeCart } from '../store/sliceCart';
function DetailSingle({ currentProduct }) {
	const dispatch = useDispatch();
	const [currentImage, setCurrentImage] = useState(0);

	return (
		<div className='h-full flex-col lg:flex-row items-center md:items-start lg:min-h-[580px] border border-[#dee2e7] bg-white rounded-xl p-[20px] mt-[20px] flex gap-5 w-full justify-between'>
			<div className='w-full lg:w-[30%]'>
				{currentProduct && (
					<img
						src={currentProduct.images[currentImage]}
						alt='radi'
						className='min-w-full h-[300px] object-contain'
					/>
				)}
				<div className='flex items-center mt-[20px] gap-2'>
					{currentProduct?.images.map((el, index) => {
						return (
							<img
								className='w-full h-[60px] object-cover border border-[#dee2e7]'
								key={index}
								src={el}
								alt={index}
			...
			<div className='w-full lg:w-[40%]'>
				{currentProduct?.stock ? (
					<p className='flex items-center gap-2 text-[#00B517]'>
				...
				<div className='flex flex-col gap-2 mt-5'>
					<p className='text-[#8B96A5] font-[300]'>
						Price: Negotiable
					</p>
					<hr />
					<p className='text-[#8B96A5] font-[300]'>
						Type: Classic {currentProduct?.title}
					</p>
					<p className='text-[#8B96A5] font-[300]'>
						Design: Modern {currentProduct?.brand}
					</p>
				</div>
				<div className='mt-10'>
					{/* put in store this current article */}
					<Link
						to='/cart'
						className='p-4 text-white bg-blue-300 rounded-lg'
						onClick={() => dispatch(storeCart(currentProduct))}>
						Add to Card
					</Link>
				</div>
			</div>
